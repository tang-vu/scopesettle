...
| API abuse / large diffs                     | Authentication for mutations, byte/file/time limits, pagination caps, rate-limit handling                       | Public endpoints can still be DoS targets without edge controls      |
...
| Frontend transaction spoofing               | Review exact chain, contract, method, budget, token, and hashes before wallet call                              | Compromised frontend can still mislead; users should inspect wallets |
...
| Secret leakage                              | Server-only modules, env names only, redacted logs, secret scan in review/CI                                    | Host compromise exposes runtime secrets                              |
